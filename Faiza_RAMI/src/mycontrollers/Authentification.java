...
public class Authentification extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(p==null) {
			response.sendRedirect("http://localhost:8282/Faiza_RAMI/authentification");
		}else {
			HttpSession session = request.getSession();
			session.setAttribute("personne",p);
			if(p.getRole().equals(Role.ADMIN)) {
				response.sendRedirect("http://localhost:8282/Faiza_RAMI/admin");
			}else {
				response.sendRedirect("http://localhost:8282/Faiza_RAMI/etudiant");
			}
		}
	}
